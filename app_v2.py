...
def ask_hh():
    with st.beta_expander("RESPONDENT"):
        d_hh = info_spouse()
...
def info_spouse(which='first'):
    ...
    colret, colcpp = st.beta_columns(2)
    with colret:
        d['ret_age'] = st.number_input("Retirement Age", min_value=2021-d['byear']+1,
                                       key="ret_age_"+which, value=65)
    with colcpp:
        d['claim_age_cpp'] = np.clip(d['ret_age'], 60, 70)
        st.write(f"\n claim age cpp: {d['claim_age_cpp']} ({message_cpp(d['ret_age'])})")
        st.write(f"\nclaim age OAS: 65")
    coled, colwage = st.beta_columns(2)
    with coled:
        d['education'] = st.selectbox("Education (highest degree obtained)",
                                      ["university", "post-secondary", "high school", "less than high school"],
                                      key="education_"+which)
    with colwage:
        d['init_wage'] = st.number_input("Annual Earnings for 2018", step=500,  min_value=0,
                                         key="init_wage_"+which, value=50000)

    colpr, colpv = st.beta_columns(2)
    with colpr:
        pension = st.radio("Do you currently (in 2020) receive a pension?", ["Yes", "No"],
                           key="pension_radio_"+which, index=1)
    if pension == "Yes":
        with colpv:
            d['pension'] = st.number_input("Yearly amount of pension",  min_value=0.0,
                                           step=0.5, key="pension_"+which)

    savings_plan = st.radio("Do you have any savings or plan to save in the future?",
                            ["Yes", "No"], key="savings_plan_"+which, index=1)
    if savings_plan == "Yes":
    ...
    db_pension = st.radio("Will you receive a DB pension from your current or previous employer",
                          ["Yes", "No"], key="db_pension_"+which, index=1)
    if db_pension == "Yes":
        st.markdown("### DB Pension")
        _, dbp1, _, dbp2, _, dbp3 = st.beta_columns([0.01, 0.28, 0.01, 0.33, 0.01, 0.33])
        with dbp1:
        ...
        with dbp3:
            d['income_previous_db'] = st.number_input("Amount of DB pension from previous employer",
                                                      min_value=0.0, step=0.5, key="income_previous_db_"+which)
    ...
    if dc_pension == "Yes":
        ...
        dcp1, _, dcp2, _, dcp3, _ = st.beta_columns([0.3, 0.01, 0.33, 0.01, 0.33, 0.02])
        with dcp1:
            d['init_dc'] = st.number_input("Annual amount of current pension", min_value=0,
                                           step=100, value=0, key="init_dc_" + which)
        with dcp2:
        ...
        if d['rate_employee_dc'] + d['rate_employer_dc'] > 0.18:
            st.write("WARNING: tax legislation caps the combined employee-employer contribution rate at 18% of earnings")
...
def debts():
    ...
    with colccd1:
        d_debts["credit_card"] = st.number_input("Balance", min_value=0.0, step=0.5, key="debt_credit_card")
    with colccd2:
        d_debts["credit_card_payment"] = st.number_input("Monthly payment", min_value=0.0, step=0.5, key="debt_payment_credit_card", max_value=d_debts["credit_card"])

    st.markdown("### Personal Loan")
    colpl1, colpl2 = st.beta_columns(2)
    with colpl1:
        d_debts["personal_loan"] = st.number_input("Balance", step=0.5, key="debt_personal_loan")
    with colpl2:
        d_debts["personal_loan_payment"] = st.number_input("Monthly payment", min_value=0.0, step=0.5, key="debt_payment_personal_loan", max_value=d_debts["personal_loan"])

    st.markdown("### Student Loan")
    colsl1, colsl2 = st.beta_columns(2)
    with colsl1:
        d_debts["student_loan"] = st.number_input("Balance", min_value=0.0, step=0.5, key="debt_student_loan")
    with colsl2:
        d_debts["student_loan_payment"] = st.number_input("Monthly payment", min_value=0.0, step=0.5, key="debt_payment_student_loan", max_value=d_debts["student_loan"])

    st.markdown("### Car Loan")
    colcl1, colcl2 = st.beta_columns(2)
    with colcl1:
        d_debts["car_loan"] = st.number_input("Balance", min_value=0.0, step=0.5, key="debt_car_loan")
    with colcl2:
        d_debts["car_loan_payment"] = st.number_input("Monthly payment", min_value=0.0, step=0.5, key="debt_payment_car_loan", max_value=d_debts["car_loan"])

    st.markdown("### Credit Line")
    colcline1, colcline2 = st.beta_columns(2)
    with colcline1:
        d_debts["credit_line"] = st.number_input("Balance", min_value=0.0, step=0.5, key="debt_credit_line")
    with colcline2:
        d_debts["credit_line_payment"] = st.number_input("Monthly payment", min_value=0.0, step=0.5, key="debt_payment_credit_line", max_value=d_debts["credit_line"])

    st.markdown("### Other debt")
    colod1, colod2 = st.beta_columns(2)
    with colod1:
        d_debts["other_debt"] = st.number_input("Balance", min_value=0.0, step=0.5, key="debt_other_debt")
    with colod2:
        d_debts["other_debt_payment"] = st.number_input("Monthly payment", min_value=0.0, step=0.5, key="debt_payment_other_debt", max_value=d_debts["other_debt"])
...
def info_residence(which):
    ...
    with colir1:
        res_value_str = "Value"
        d_res[f'{which}_residence'] = st.number_input(res_value_str, min_value=0.0, step=0.5, key="res_value")
    with colir2:
        res_buy_str = "Buying price"
        d_res[f'price_{which}_residence'] = st.number_input(res_buy_str, min_value=0.0, step=0.5, key="res_buy")
    with colir3:
        res_mortgage_str = "Outstanding mortgage"
        d_res[f'{which}_mortgage'] = st.number_input(res_mortgage_str, min_value=0.0, step=0.5, key="res_mortgage")
    with colir4:
        res_mortgage_payment_str = "Monthly payment on mortgage"
        d_res[f'{which}_mortgage_payment'] = st.number_input(res_mortgage_payment_str, min_value=0.0, step=0.5, key="res_mortgage_payment")
    return d_res
...
def info_hh():
    ...
    if business == "Yes":
        with colb1:
            d_others['business'] = st.number_input("Value of the business in 2020", step=0.5, key="business_value")
        with colb2:
...
def fin_accounts(which):
    ...
    with colrrsp1:
        d_fin["bal_rrsp"] = st.number_input("Amount in RRSP account", min_value=0.0, step=0.5, key="bal_rrsp_"+which)
    with colrrsp2:
        d_fin["cont_rate_rrsp"] = st.number_input(
            "Fraction of your earnings you plan to save in your RRSP account (in %)",
    ...
    with colrrsp3:
        d_fin["withdrawal_rrsp"] = st.number_input("Amount of your RRSP account you plan to spend (in $)",
                                                   min_value=0.0, key="withdrawal_rrsp_"+which)
    with colrrsp4:
        d_fin["init_room_rrsp"] = st.number_input("Contribution room for RRSP in 2020", 
                                                  min_value=0.0, key="init_room_rrsp_"+which)

    st.markdown("### TFSA")
    coltfsa1, coltfsa2 = st.beta_columns(2)
    coltfsa3, coltfsa4 = st.beta_columns(2)
    with coltfsa1:
        d_fin["bal_tfsa"] = st.number_input("Amount in TFSA account", min_value=0.0, step=0.5, key="bal_tfsa_"+which)
    with coltfsa2:
    ...
    with coltfsa3:
        d_fin["withdrawal_tfsa"] = st.number_input("Amount of your TFSA account you plan to spend (in $)", min_value=0.0, key="withdrawal_tfsa_"+which)
    with coltfsa4:
    ...
    with coloreg1:
        d_fin["bal_other_reg"] = st.number_input("Amount in Other Reg account", min_value=0.0, step=0.5, key="bal_other_reg_"+which)
    with coloreg2:
    ...
    with coloreg3:
        d_fin["withdrawal_other_reg"] = st.number_input("Amount of your Other Reg account you plan to spend (in $)",
                                                        min_value=0.0, key="withdrawal_other_reg_"+which)

    st.markdown("### Unreg")
    colureg1, colureg2 = st.beta_columns(2)
    colureg3, colureg4 = st.beta_columns(2)
    with colureg1:
        d_fin["bal_unreg"] = st.number_input("Amount in Unreg account", step=0.5,
                                             key="bal_unreg_"+which)
    with colureg2:
        d_fin["cont_rate_unreg"] = st.number_input(
            "Fraction of your earnings you plan to save in your Unreg account (in %)",
    ...
    with colureg3:
        d_fin["withdrawal_unreg"] = st.number_input("Amount of your Unreg account you plan to spend (in $)",
                                                    min_value=0.0, key="withdrawal_unreg_"+which)
...
#cbc1, cbc2 = st.beta_columns(2)

#with cbc1:
#	if st.checkbox("Show raw data", False):
#		st.write(d_hh)

#with open('py/d_hh.pkl', 'rb') as file:
#	d_hhd = pickle.load(file)
#dfd = create_dataframe(d_hhd)

#with cbc2:
#	if st.checkbox("Show raw data from dict", False):
#		st.write(d_hhd)

if st.button("Show visualizations", False):
    #try:
    #	df_res = prepare_RRI(df)
        #st.write(prepare_RRI(dfd))
        #st.write(prepare_RRI(df))
    #except:
    #	if df["couple"][0] == False:
    #		st.error("Please select **'Yes'** in **'Do you have any savings or plan to save in the future?'** for the **first spouse** and **fill the data**.")
            #st.warning("Note: You can keep the values as 0 if you don't have any savings.")
    #	else:
    #		st.error("Please select **'Yes'** in **'Do you have any savings or plan to save in the future?'** for both **first and second spouse** and **fill the data**.")
            #st.warning("Note: You can keep the values as 0 if you don't have any savings.")
    #	st.stop()
    df_res = prepare_RRI(df)
